...
def unpermute_with_padded_tokens(
    ...
    empty_tokens = torch.zeros(restore_shape, dtype=combined_output.dtype, device=combined_output.device)
...
def moe_forward(self, hidden_states):
    ...
    final_hidden_states = nnscaler_moe_gmm(
        hidden_states, selected_experts, routing_weights, gate_projs, up_projs, down_projs, 8, 0, 128
    )
